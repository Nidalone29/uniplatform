...
export function DeleteDialog<T extends Entities>({ data }: DeleteDialogProps<T>) {
  ...
  function onSubmit() {
    ...
    submitFunction({ intent: "delete", slug: data.slug }, { method: "POST", encType: "application/json" });
  }
  ...
  return (
    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Are you absolutely sure you want to delete {data.name}?</DialogTitle>
        </DialogHeader>
        <p>This will also delete any courses or exams associated with {data.name}, if there are any.</p>
        <DialogFooter>
          <DialogClose asChild>
            <Button variant="outline">Cancel</Button>
          </DialogClose>
          <Button type="submit" onClick={() => onSubmit()} disabled={formState}>
            Delete {formState ? <Loader2Icon className="animate-spin" /> : <></>}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
