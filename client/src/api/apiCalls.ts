...
export async function getUniversity(uni: string): Promise<University> {
  return await ky("http://localhost:8080/api/universities/" + uni).json<University>();
}
...
export async function getCourse(uni: string, course: string): Promise<Course> {
  return await ky("http://localhost:8080/api/universities/" + uni + "/courses/" + course).json<Course>();
}
